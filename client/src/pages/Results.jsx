...
export default function Results() {
  ...
  useEffect(() => {
    ...
    if (niveau === "Lycee" && serie) params.serie = serie;
    api.getResults(params).then(setRows).catch((e) => setError(e.message));
  }, [niveau, classe, serie]);
  ...
  return (
    <div>
      <header className="page-header">
        ...
        <p className="page-subtitle">Moyennes, rang et mentions, calculés en temps réel.</p>
      </header>
          ...
          <table>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.ID}>
                  <td>
                    <div className="rank-seal" style={{ width: 30, height: 30, fontSize: "0.85rem" }}>{r.rang}</div>
                  </td>
